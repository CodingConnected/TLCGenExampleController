...
// wordt bij ISG hier opgenomen. Betekent ook dat er een aparte projectfile moet zijn voor ISG maar laten we nu nog even aan de gebruiker
...
void PreApplication(void)
{
    ...
    if (IH[homschtegenh]) /* tegenhouden inschakelen naar PL */
    {
        ...
        if (!(T[tlr3231] || RT[tlr3231])) RR[fc32] |= RR_INSCH_HALFSTAR;
        if (!(T[tlr3132] || RT[tlr3132])) RR[fc31] |= RR_INSCH_HALFSTAR;
        if (!(T[tlr3433] || RT[tlr3433])) RR[fc34] |= RR_INSCH_HALFSTAR;
        if (!(T[tlr3334] || RT[tlr3334])) RR[fc33] |= RR_INSCH_HALFSTAR;
    ...
    if (!IH[hkpact] && !IH[hpervar] && !SCH[schvar] && !IH[hplhd])
    {
        if (!T[tnlfg0262] && !RT[tnlfg0262] && !T[tnleg0262] && !RT[tnleg0262] && !T[tnlegd0262] && !RT[tnlegd0262] && !T[tnlfgd0262] && !RT[tnlfgd0262] &&
            ...
            !T[tlr3132] && !RT[tlr3132]  &&
            !T[tlr3231] && !RT[tlr3231]  &&
            !T[tlr3334] && !RT[tlr3334]  &&
            !T[tlr3433] && !RT[tlr3433]  &&
    ...
    //@Menno: deze komt ergens van beneden deze file. De volgorde heeft PeterS gewijzigd. 
...
void Aanvragen(void)
{
    ...
    IH[hmadk34b] = G[fc34] && !SG[fc34] ? FALSE : IH[hmadk34b] || D[dk34b] && A[fc34];
        ...
        if (ris_aanvraag(fc03, SYSTEM_ITF1, PRM[prmrislaneid03_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm003mveh1], PRM[prmrisaendsrm003mveh1], !SCH[schrisgeencheckopsg])) A[fc03] |= BIT13;
        ...
        if (ris_aanvraag(fc05, SYSTEM_ITF1, PRM[prmrislaneid05_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm005mveh1], PRM[prmrisaendsrm005mveh1], !SCH[schrisgeencheckopsg])) A[fc05] |= BIT13;
        ...
        if (ris_aanvraag(fc08, SYSTEM_ITF1, PRM[prmrislaneid08_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm008mveh1], PRM[prmrisaendsrm008mveh1], !SCH[schrisgeencheckopsg])) A[fc08] |= BIT13;
        ...
        if (ris_aanvraag(fc08, SYSTEM_ITF1, PRM[prmrislaneid08_2], RIS_MOTORVEHICLES, PRM[prmrisastartsrm008mveh2], PRM[prmrisaendsrm008mveh2], !SCH[schrisgeencheckopsg])) A[fc08] |= BIT13;
        ...
        if (ris_aanvraag(fc33, SYSTEM_ITF1, PRM[prmrislaneid33_1], RIS_PEDESTRIAN, PRM[prmrisastartsrm033vtg1], PRM[prmrisaendsrm033vtg1], !SCH[schrisgeencheckopsg])) A[fc33] |= BIT13;
        ...
        if (ris_aanvraag(fc61, SYSTEM_ITF1, PRM[prmrislaneid61_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm061mveh1], PRM[prmrisaendsrm061mveh1], !SCH[schrisgeencheckopsg])) A[fc61] |= BIT13;
        ...
        if (ris_aanvraag(fc62, SYSTEM_ITF1, PRM[prmrislaneid62_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm062mveh1], PRM[prmrisaendsrm062mveh1], !SCH[schrisgeencheckopsg])) A[fc62] |= BIT13;
        ...
        if (ris_aanvraag(fc62, SYSTEM_ITF1, PRM[prmrislaneid62_2], RIS_MOTORVEHICLES, PRM[prmrisastartsrm062mveh2], PRM[prmrisaendsrm062mveh2], !SCH[schrisgeencheckopsg])) A[fc62] |= BIT13;
        ...
        if (ris_aanvraag(fc67, SYSTEM_ITF1, PRM[prmrislaneid67_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm067mveh1], PRM[prmrisaendsrm067mveh1], !SCH[schrisgeencheckopsg])) A[fc67] |= BIT13;
        ...
        if (ris_aanvraag(fc68, SYSTEM_ITF1, PRM[prmrislaneid68_1], RIS_MOTORVEHICLES, PRM[prmrisastartsrm068mveh1], PRM[prmrisaendsrm068mveh1], !SCH[schrisgeencheckopsg])) A[fc68] |= BIT13;
        ...
        if (ris_aanvraag(fc68, SYSTEM_ITF1, PRM[prmrislaneid68_2], RIS_MOTORVEHICLES, PRM[prmrisastartsrm068mveh2], PRM[prmrisaendsrm068mveh2], !SCH[schrisgeencheckopsg])) A[fc68] |= BIT13;
        ...
        if (ris_aanvraag(fc81, SYSTEM_ITF1, PRM[prmrislaneid81_1], RIS_CYCLIST, PRM[prmrisastartsrm081fts1], PRM[prmrisaendsrm081fts1], !SCH[schrisgeencheckopsg])) A[fc81] |= BIT13;
        ...
        if (ris_aanvraag(fc82, SYSTEM_ITF1, PRM[prmrislaneid82_1], RIS_CYCLIST, PRM[prmrisastartsrm082fts1], PRM[prmrisaendsrm082fts1], !SCH[schrisgeencheckopsg])) A[fc82] |= BIT13;
...
{
   //@Menno: count i en j heeft PeterS weggehaald. Even kijken waar ze vandaan komen. 
    ...
    //@ Menno: deze is omgedraaid. In de sys.h / tab.c is de naam hier ook. Dit moet waarschijnljk ook in de REALFUNC. Hierboven gaat het wel goed met de voorstart. 
    Realisatietijd_Ontruiming_LateRelease(fc11, fc26, tlr2611, tfo1126);
    ...
    if (SCH[schgs2232]) wijziging |= Realisatietijd_Gelijkstart_Correctie(fc32, fc22);
    if (SCH[schgs2434]) wijziging |= Realisatietijd_Gelijkstart_Correctie(fc34, fc24);
    if (SCH[schgs2484]) wijziging |= Realisatietijd_Gelijkstart_Correctie(fc84, fc24);
    wijziging |= Realisatietijd_Gelijkstart_Correctie(fc38, fc28);
    if (SCH[schgs3384]) wijziging |= Realisatietijd_Gelijkstart_Correctie(fc84, fc33);
    ...
    xyprintf(0, 3, "REALISATIETIJD[fc26][fc08] = %3d", REALISATIETIJD[fc26][fc08]);

    /* Inlopen / inrijden nalopen */
    wijziging |= Realisatietijd_LateRelease_Correctie(fc62, fc02, tlr6202);
    ...
    wijziging |= (!IH[hlos31]) ? Realisatietijd_LateRelease_Correctie(fc32, fc31, tlr3231) : 0;
    IH[hlos32] = RA[fc32] && (!H[hmadk32a] || SCH[schlos3231] && H[hmadk32b]) || H[hlos32] && !SG[fc32];
    wijziging |= (!IH[hlos32]) ? Realisatietijd_LateRelease_Correctie(fc31, fc32, tlr3231) : 0;
    IH[hlos33] = RA[fc33] && (!H[hmadk33a] || SCH[schlos3334] && H[hmadk33b]) || H[hlos33] && !SG[fc33];
    wijziging |= (!IH[hlos33]) ? Realisatietijd_LateRelease_Correctie(fc34, fc33, tlr3433) : 0;
    IH[hlos34] = RA[fc34] && (!H[hmadk34a] || SCH[schlos3433] && H[hmadk34b]) || H[hlos34] && !SG[fc34];
    wijziging |= (!IH[hlos34]) ? Realisatietijd_LateRelease_Correctie(fc33, fc34, tlr3334) : 0;
    wijziging |= Realisatietijd_LateRelease_Correctie(fc81, fc82, tlr8182);
...
void BepaalInterStartGroenTijden(void)
{
    ...
    do
    {
        ...
        /* Gelijkstart / voorstart / late release */
    if (SCH[schgs2232])         wijziging |= InterStartGroenTijd_Gelijkstart_Correctie(fc32, fc22);
    if (SCH[schgs2434])         wijziging |= InterStartGroenTijd_Gelijkstart_Correctie(fc34, fc24);
    if (SCH[schgs2484])         wijziging |= InterStartGroenTijd_Gelijkstart_Correctie(fc84, fc24);
            wijziging |= InterStartGroenTijd_Gelijkstart_Correctie(fc38, fc28);
    if (SCH[schgs3384])         wijziging |= InterStartGroenTijd_Gelijkstart_Correctie(fc84, fc33);
        ...
        /* Inlopen / inrijden */
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc62, fc02, tlr6202);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc68, fc08, tlr6808);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc68, fc11, tlr6811);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc21, fc22, tlr2122);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc32, fc31, tlr3231);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc31, fc32, tlr3132);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc34, fc33, tlr3433);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc33, fc34, tlr3334);
        wijziging |= InterStartGroenTijd_LateRelease_Correctie(fc81, fc82, tlr8182);
...
void BepaalInterStartGroenTijden_PRIO(void)
{
    ...
    do
    {
        wijziging = FALSE;
        /* Pas realisatietijden aan a.g.v. nalopen */
        if (SCH[schgs2232]) wijziging |= TISG_Gelijkstart_PRIO_Correctie(fc22, fc32);
        if (SCH[schgs2434]) wijziging |= TISG_Gelijkstart_PRIO_Correctie(fc24, fc34);
        if (SCH[schgs2484]) wijziging |= TISG_Gelijkstart_PRIO_Correctie(fc24, fc84);
        wijziging |= TISG_Gelijkstart_PRIO_Correctie(fc28, fc38);
        if (SCH[schgs3384]) wijziging |= TISG_Gelijkstart_PRIO_Correctie(fc33, fc84);
        ...
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc62, fc02, tlr6202);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc68, fc08, tlr6808);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc68, fc11, tlr6811);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc21, fc22, tlr2122);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc81, fc82, tlr8182);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc32, fc31, tlr3231);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc31, fc32, tlr3132);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc34, fc33, tlr3433);
        wijziging |= TISG_LateRelease_PRIO_Correctie(fc33, fc34, tlr3334);
...
void Verlenggroen(void)
{
    ...
    //@Menno: NG = -1 dus dat gaat het fout. Het moet dus 0 zijn. Ivm goed werkende wachttijdvoorspeller.
    TVG_AR[fc02] = ((PRM[prmaltg02] - TFG_max[fc02]) >= 0) ? PRM[prmaltg02] - TFG_max[fc02] : 0;
    TVG_AR[fc03] = ((PRM[prmaltg03] - TFG_max[fc03]) >= 0) ? PRM[prmaltg03] - TFG_max[fc03] : 0;
    TVG_AR[fc05] = ((PRM[prmaltg05] - TFG_max[fc05]) >= 0) ? PRM[prmaltg05] - TFG_max[fc05] : 0;
    TVG_AR[fc08] = ((PRM[prmaltg08] - TFG_max[fc08]) >= 0) ? PRM[prmaltg08] - TFG_max[fc08] : 0;
    TVG_AR[fc09] = ((PRM[prmaltg09] - TFG_max[fc09]) >= 0) ? PRM[prmaltg09] - TFG_max[fc09] : 0;
    TVG_AR[fc11] = ((PRM[prmaltg11] - TFG_max[fc11]) >= 0) ? PRM[prmaltg11] - TFG_max[fc11] : 0;
    TVG_AR[fc21] = ((PRM[prmaltg21] - TFG_max[fc21]) >= 0) ? PRM[prmaltg21] - TFG_max[fc21] : 0;
    TVG_AR[fc22] = ((PRM[prmaltg22] - TFG_max[fc22]) >= 0) ? PRM[prmaltg22] - TFG_max[fc22] : 0;
    TVG_AR[fc24] = ((PRM[prmaltg24] - TFG_max[fc24]) >= 0) ? PRM[prmaltg24] - TFG_max[fc24] : 0;
    TVG_AR[fc26] = ((PRM[prmaltg26] - TFG_max[fc26]) >= 0) ? PRM[prmaltg26] - TFG_max[fc26] : 0;
    TVG_AR[fc28] = ((PRM[prmaltg28] - TFG_max[fc28]) >= 0) ? PRM[prmaltg28] - TFG_max[fc28] : 0;
    TVG_AR[fc31] = ((PRM[prmaltg31] - TFG_max[fc31]) >= 0) ? PRM[prmaltg31] - TFG_max[fc31] : 0;
    TVG_AR[fc32] = ((PRM[prmaltg32] - TFG_max[fc32]) >= 0) ? PRM[prmaltg32] - TFG_max[fc32] : 0;
    TVG_AR[fc33] = ((PRM[prmaltg33] - TFG_max[fc33]) >= 0) ? PRM[prmaltg33] - TFG_max[fc33] : 0;
    TVG_AR[fc34] = ((PRM[prmaltg34] - TFG_max[fc34]) >= 0) ? PRM[prmaltg34] - TFG_max[fc34] : 0;
    TVG_AR[fc38] = ((PRM[prmaltg38] - TFG_max[fc38]) >= 0) ? PRM[prmaltg38] - TFG_max[fc38] : 0;
    TVG_AR[fc61] = ((PRM[prmaltg61] - TFG_max[fc61]) >= 0) ? PRM[prmaltg61] - TFG_max[fc61] : 0;
    TVG_AR[fc62] = ((PRM[prmaltg62] - TFG_max[fc62]) >= 0) ? PRM[prmaltg62] - TFG_max[fc62] : 0;
    TVG_AR[fc67] = ((PRM[prmaltg67] - TFG_max[fc67]) >= 0) ? PRM[prmaltg67] - TFG_max[fc67] : 0;
    TVG_AR[fc68] = ((PRM[prmaltg68] - TFG_max[fc68]) >= 0) ? PRM[prmaltg68] - TFG_max[fc68] : 0;
    TVG_AR[fc81] = ((PRM[prmaltg81] - TFG_max[fc81]) >= 0) ? PRM[prmaltg81] - TFG_max[fc81] : 0;
    TVG_AR[fc82] = ((PRM[prmaltg82] - TFG_max[fc82]) >= 0) ? PRM[prmaltg82] - TFG_max[fc82] : 0;
    TVG_AR[fc84] = ((PRM[prmaltg84] - TFG_max[fc84]) >= 0) ? PRM[prmaltg84] - TFG_max[fc84] : 0;
    ...
    */
    ...
    VerhoogGroentijdNietTijdensInrijden(fc02, fc62, tlr6202);
    VerhoogGroentijdNietTijdensInrijden(fc08, fc68, tlr6808);
    VerhoogGroentijdNietTijdensInrijden(fc11, fc68, tlr6811);
    VerhoogGroentijdNietTijdensInrijden(fc22, fc21, tlr2122);
    VerhoogGroentijdNietTijdensInrijden(fc31, fc32, tlr3231);
    VerhoogGroentijdNietTijdensInrijden(fc32, fc31, tlr3132);
    VerhoogGroentijdNietTijdensInrijden(fc33, fc34, tlr3433);
    VerhoogGroentijdNietTijdensInrijden(fc34, fc33, tlr3334);
    VerhoogGroentijdNietTijdensInrijden(fc82, fc81, tlr8182);
...
void Meetkriterium(void)
{
    ...
    if (SCH[schschoolingreep34] && H[hschoolingreepdk34b] && T[tschoolingreepmaxg34]) RW[fc34] |= BIT8;
        ...
        if (ris_verlengen(fc03, SYSTEM_ITF1, PRM[prmrislaneid03_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm003mveh1], PRM[prmrisvendsrm003mveh1], !SCH[schrisgeencheckopsg])) MK[fc03] |= BIT13;
        ...
        if (ris_verlengen(fc05, SYSTEM_ITF1, PRM[prmrislaneid05_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm005mveh1], PRM[prmrisvendsrm005mveh1], !SCH[schrisgeencheckopsg])) MK[fc05] |= BIT13;
        ...
        if (ris_verlengen(fc08, SYSTEM_ITF1, PRM[prmrislaneid08_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm008mveh1], PRM[prmrisvendsrm008mveh1], !SCH[schrisgeencheckopsg])) MK[fc08] |= BIT13;
        ...
        if (ris_verlengen(fc08, SYSTEM_ITF1, PRM[prmrislaneid08_2], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm008mveh2], PRM[prmrisvendsrm008mveh2], !SCH[schrisgeencheckopsg])) MK[fc08] |= BIT13;
        ...
        if (ris_verlengen(fc33, SYSTEM_ITF1, PRM[prmrislaneid33_1], RIS_PEDESTRIAN, PRM[prmrisvstartsrm033vtg1], PRM[prmrisvendsrm033vtg1], !SCH[schrisgeencheckopsg])) MK[fc33] |= BIT13;
        ...
        if (ris_verlengen(fc61, SYSTEM_ITF1, PRM[prmrislaneid61_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm061mveh1], PRM[prmrisvendsrm061mveh1], !SCH[schrisgeencheckopsg])) MK[fc61] |= BIT13;
        ...
        if (ris_verlengen(fc62, SYSTEM_ITF1, PRM[prmrislaneid62_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm062mveh1], PRM[prmrisvendsrm062mveh1], !SCH[schrisgeencheckopsg])) MK[fc62] |= BIT13;
        ...
        if (ris_verlengen(fc62, SYSTEM_ITF1, PRM[prmrislaneid62_2], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm062mveh2], PRM[prmrisvendsrm062mveh2], !SCH[schrisgeencheckopsg])) MK[fc62] |= BIT13;
        ...
        if (ris_verlengen(fc67, SYSTEM_ITF1, PRM[prmrislaneid67_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm067mveh1], PRM[prmrisvendsrm067mveh1], !SCH[schrisgeencheckopsg])) MK[fc67] |= BIT13;
        ...
        if (ris_verlengen(fc68, SYSTEM_ITF1, PRM[prmrislaneid68_1], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm068mveh1], PRM[prmrisvendsrm068mveh1], !SCH[schrisgeencheckopsg])) MK[fc68] |= BIT13;
        ...
        if (ris_verlengen(fc68, SYSTEM_ITF1, PRM[prmrislaneid68_2], RIS_MOTORVEHICLES, PRM[prmrisvstartsrm068mveh2], PRM[prmrisvendsrm068mveh2], !SCH[schrisgeencheckopsg])) MK[fc68] |= BIT13;
        ...
        if (ris_verlengen(fc81, SYSTEM_ITF1, PRM[prmrislaneid81_1], RIS_CYCLIST, PRM[prmrisvstartsrm081fts1], PRM[prmrisvendsrm081fts1], !SCH[schrisgeencheckopsg])) MK[fc81] |= BIT13;
        ...
        if (ris_verlengen(fc82, SYSTEM_ITF1, PRM[prmrislaneid82_1], RIS_CYCLIST, PRM[prmrisvstartsrm082fts1], PRM[prmrisvendsrm082fts1], !SCH[schrisgeencheckopsg])) MK[fc82] |= BIT13;
...
void RealisatieAfhandeling(void)
{
    ...
    /*@Menno: Als je wachttijdvoorspeller hebt mag je hem niet meer terughalen.... */
/*    RR[fc02] |= R[fc02] && AR[fc02] && (!PAR[fc02] || ERA[fc02]) ? BIT5 : 0;
    ...
    if (!(RR[fc22] & BIT5)) RR[fc32] &= ~BIT5;
    if (!(RR[fc32] & BIT5)) RR[fc22] &= ~BIT5;
    if (!(RR[fc24] & BIT5)) RR[fc34] &= ~BIT5;
    if (!(RR[fc34] & BIT5)) RR[fc24] &= ~BIT5;
    if (!(RR[fc24] & BIT5)) RR[fc84] &= ~BIT5;
    if (!(RR[fc84] & BIT5)) RR[fc24] &= ~BIT5;
    if (!(RR[fc28] & BIT5)) RR[fc38] &= ~BIT5;
    if (!(RR[fc38] & BIT5)) RR[fc28] &= ~BIT5;
    if (!(RR[fc33] & BIT5)) RR[fc84] &= ~BIT5;
    if (!(RR[fc84] & BIT5)) RR[fc33] &= ~BIT5;
    ...
    PAR[fc22] = max_par(fc22, PRML, ML) && SCH[schaltg2232];
    PAR[fc24] = max_par(fc24, PRML, ML) && SCH[schaltg243484];
    PAR[fc26] = max_par(fc26, PRML, ML) && SCH[schaltg26];
    PAR[fc28] = max_par(fc28, PRML, ML) && SCH[schaltg2838];
    PAR[fc31] = max_par(fc31, PRML, ML) && SCH[schaltg31];
    PAR[fc32] = max_par(fc32, PRML, ML) && SCH[schaltg2232];
    PAR[fc33] = max_par(fc33, PRML, ML) && SCH[schaltg3384];
    PAR[fc34] = max_par(fc34, PRML, ML) && SCH[schaltg2434];
    PAR[fc38] = max_par(fc38, PRML, ML) && SCH[schaltg2838];
    ...
    PAR[fc84] = max_par(fc84, PRML, ML) && SCH[schaltg243384];
    ...
    /* @Menno toegevoegde code; oo khet hulpelement moet worden aangemaakt (nu door Peter met de hand gedaan). Geldt voor alle koppelingen (startgoren / einde groetn etc. */
    IH[hnleg0262] = !SCH[schlos0262];
    IH[hnleg0868] = !SCH[schlos0868];
    IH[hnleg1168] = !SCH[schlos1168];
    IH[hnleg2221] = !SCH[schlos2221];
    ...
    IH[hnleg8281] = !SCH[schlos8281];

    /* PAR-ongecoordineerd */
    /* @Menno aangepaste code ivm schakelaar. gewoon overnemen zo. */
    if (!PAR[fc31] && SCH[schlos3132] && max_par_los(fc31) && (!IH[hmadk31a] || !SCH[schlosgeennla3132_2]) || PAR_los[fc31] && RA[fc31]) PAR_los[fc31] = TRUE; else PAR_los[fc31] = FALSE;
    if (!PAR[fc32] && SCH[schlos3231] && max_par_los(fc32) && (!IH[hmadk32a] || !SCH[schlosgeennla3231_2]) || PAR_los[fc32] && RA[fc32]) PAR_los[fc32] = TRUE; else PAR_los[fc32] = FALSE;
    if (!PAR[fc33] && SCH[schlos3334] && max_par_los(fc33) && (!IH[hmadk33a] || !SCH[schlosgeennla3334_2]) || PAR_los[fc33] && RA[fc33]) PAR_los[fc33] = TRUE; else PAR_los[fc33] = FALSE;
    if (!PAR[fc34] && SCH[schlos3433] && max_par_los(fc34) && (!IH[hmadk34a] || !SCH[schlosgeennla3433_2]) || PAR_los[fc34] && RA[fc34]) PAR_los[fc34] = TRUE; else PAR_los[fc34] = FALSE;
...
void application(void)
{
    ...
    {
       /* @Menno: aangepast volgorde van aanroepen. */
        Meetkriterium();
        Verlenggroen();
        Wachtgroen();
...
#ifndef NO_PRIO
    /* Star programma mag je niet gebruiken bij ISG nog. Moeten we in de GUI uitzetten. */
    if (MM[mstarprog] == 0 && (IH[hmlact] || SCH[schovpriople]) && FALSE) AfhandelingPrio();
    else
    {
        int fc;
        RTFB &= ~PRIO_RTFB_BIT;
...
#endif /* NO_PRIO */
//@Menno: deze functie schakelbaar ivm snelheid. 
#ifndef AUTOMAAT
...
void system_application(void)
{
    ...
    //@Menno: code wachttijdvoorspellers was weggevallen. Hier weer opgenomem. 
...
void system_application2(void)
{
    ...
    mon3_mon4_buffers(SAPPLPROG, PRM[prmmaxtvg], PRM[prmmaxtfbvlog]);
        ...
        for (i = 0; i < FCMAX; ++i)
        {
            timings_uc4(i, t_wacht[i], NG, NG, prmttxconfidence15, schtxconfidence15ar, schtimings02 + i);
        }
        if (!SCH[schconfidence15fix])
        {
            for (i = 0; i < FCMAX; ++i)
